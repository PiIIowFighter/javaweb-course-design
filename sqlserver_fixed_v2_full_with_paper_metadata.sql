...
-- Files：稿件附件/多附件（例如 Cover Letter 附件）
...
/* ============================================================
   ADD-ON (2026-01-08): Manuscripts publication metadata
   Purpose:
     - Add CNKI-like publication metadata columns for ACCEPTED manuscripts
     - Backfill missing values (idempotent)
     - Set CNKI URL for ManuscriptId=15 if empty (idempotent)
   Notes:
     - Split into multiple batches with GO to avoid "invalid column" compile-time errors
   ============================================================ */
...
/* 2) Backfill ACCEPTED manuscripts (only fill missing values; do not overwrite) */
IF OBJECT_ID(N'dbo.Manuscripts', N'U') IS NOT NULL
BEGIN
    UPDATE m
...
/* 3) Special: CNKI URL for ManuscriptId=15 (only if empty) */
IF OBJECT_ID(N'dbo.Manuscripts', N'U') IS NOT NULL
BEGIN
    UPDATE dbo.Manuscripts
